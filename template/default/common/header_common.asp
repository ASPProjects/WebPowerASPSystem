<!DOCTYPE HTML>
<html>
<head>
	<meta charset="UTF-8" />
	<meta http-equiv="X-UA-Compatible" content="IE=EDGE,CHROME=1" />
	<title><% =SeoTitle %> - <% =SiteTitle %></title>
	<meta name="keywords" content="<% =Keywords %>" />
	<meta name="description" content="<% =Descriptions %>" />
	<link rel="stylesheet" type="text/css" href="static/css/global_reset.css" />
	<link rel="stylesheet" type="text/css" href="static/css/main.css"/>
	<script type="text/javascript" src="static/js/jquery.js"></script>
	<!--[if lt IE 7]><script type="text/javascript" src="static/lib/unitpngfix/unitpngfix.js"></script><![endif]-->
</head>
<body>
<div id="header">
<div class="wp cl">
<div id="logo"></div>
<div id="intro" class="cl">
<div class="light">
<span>专业生产室内外金属铝单板、铝天花板、铝蜂窝板、仿大理石铝单板、</span>
<span>镂空铝单板、造型铝单板、仿木纹铝单板、瓦楞板、真石漆铝单板、</span>
<span>自洁铝单板、U型方通，铝挂条、铝百叶及铝型材喷涂加工。</span>
</div>
<div class="front">
<span>专业生产室内外金属铝单板、铝天花板、铝蜂窝板、仿大理石铝单板、</span>
<span>镂空铝单板、造型铝单板、仿木纹铝单板、瓦楞板、真石漆铝单板、</span>
<span>自洁铝单板、U型方通，铝挂条、铝百叶及铝型材喷涂加工。</span>
</div>
</div>
<div id="fav">
<a href="#">设为主页</a>|
<a href="#">加入收藏</a>
</div>
</div>
<div id="nav" class="wp cl">
<a href="index.asp">首页   </a>
<a href="aboutus.asp">关于我们</a>
<a href="productlist.asp">产品中心</a>
<a href="caselist.asp">工程案例</a>
<a href="newslist.asp">新闻资讯</a>
<a href="marnet.asp">销售网络</a>
<a href="hiring.asp">招聘信息</a>
<a href="feedback.asp">信息反馈</a>
<a href="contactus.asp">联系我们</a>
</div>
</div>
<div style="overflow: hidden;">
<div id="banner" class="wp">
<img src="static/image/banner.jpg"/>
</div>
</div>
<div id="ctTop" class="wp"></div>